"""
algorithm_tmdb_similar.py
-------------------------
Purpose:
    Fetch TMDB "similar" titles for a given movie id from /movie/{id}/similar
...
Output:
    - Writes: python_scripts/assets/schema_tmdb_similar.md
    - Also prints a compact table to the terminal.

Usage (from project root):
    python -m python_scripts.tmdb.algorithm_tmdb_similar

Notes:
    - Requires TMDB_API_KEY in .env
    - Uses language from shared/constants
"""
...
def main():
    ...
    url = f"https://api.themoviedb.org/3/movie/{EXAMPLE_MOVIE_ID}/similar"
    params = {
        "api_key": api_key,
    ...
    # Write Markdown
    title = "TMDB Similar Titles Schema"
    source_line = (
        f"Source: TMDB /movie/{{id}}/similar with id={EXAMPLE_MOVIE_ID}, "
        f"language={DEFAULT_LANGUAGE}."
    )
    outpath = os.path.join(
        os.path.dirname(__file__), "..", "assets", "schema_tmdb_similar.md"
    )
